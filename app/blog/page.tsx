            ...
            {posts.map((post: any) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                <article className="flex flex-col">
                  {/* Image wrapper */}
                  <div className="relative aspect-[16/10] overflow-hidden mb-8 bg-secondary rounded-sm">
                    {post.featuredImage ? (
                      <img
                  ...
                  {/* Title */}
                  <h2 className="text-2xl md:text-3xl font-heading font-medium text-foreground group-hover:text-primary transition-colors duration-500 leading-snug mb-4">
                    {post.title}
                  </h2>
                  
                  {/* Date/Meta */}
                  <span className="text-foreground/30 text-[10px] uppercase tracking-widest font-medium">
                    {post.date}
                  </span>
                </article>
              </Link>
